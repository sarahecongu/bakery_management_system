...
class Cart extends Model
{
    ...
    public function reduceCartItemQuantity(CartItem $cart_items)
    {
        $session = new SessionManager();

        if ($session->checkLoginStatus()) {
            try {
                $cartId = $this->getUserCart($session->get('id'));

                // Check if the product is in the user's cart
                $existingCartItem = $cart_items->where(['cart_id' => $cartId, 'product_id' => $_POST['product_id']]);

                if ($existingCartItem) {
                    // Reduce the quantity, but not below 1
                    $newQuantity = max(1, $existingCartItem[0]->quantity - 1);
                    $cart_items->update($existingCartItem[0]->id, ['quantity' => $newQuantity]);
            ...
            if (isset($_SESSION['cart'])) {
                ...
                if ($existingCartItem) {
                    // Reduce the quantity in the session cart, but not below 1
                    $existingCartItemKey = key($existingCartItem);
                    $_SESSION['cart'][$existingCartItemKey]['quantity'] = max(1, $_SESSION['cart'][$existingCartItemKey]['quantity'] - 1);

                    // Return the updated quantity
                    return $_SESSION['cart'][$existingCartItemKey]['quantity'];
                }
            }
        }
